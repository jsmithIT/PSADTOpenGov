...
Try {
	...
	[string]$appVendor = 'OpenGov'
	[string]$appName = 'OpenGov'
	[string]$appVersion = '1.0.1'
	[string]$appArch = 'x86'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.1.0'
	[string]$appScriptDate = '2022.10.15'
	...
	$32bitAppInstallName = "OpenGov"
	# Application install Path.
	$32bitAppInstallPath = "$Env:Programfiles\Tuolumne County\OpenGov"
	# Application  install parameters.
	...
	#Directories.
	[string[]]$appShortcutDirs = @("$Env:Programfiles\Tuolumne County\OpenGov")
	[string[]]$folderShortcutDirs = @("$Env:ProgramData\Microsoft\Windows\Start Menu\Programs", "${Env:PUBLIC}\Desktop")
	...
	#Apps to be superseded by OpenGov and their corresponsing paths.
	$SupersededApp1 = "OpenGov CIT.lnk"
	$SupersededApp2 = "OpenGov Budgeting.lnk"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Foreach ($removeShortcutDir in $removeShortcutDirs) {
			...
			If (Test-Path -Path "$removeShortcutDir\$32bitAppUninstallName") {
				...
				Try {
					...
					Remove-Item -Path "$removeShortcutDir\$SupersededApp1" -Force -Recurse
					Remove-Item -Path "$removeShortcutDir\$SupersededApp2" -Force -Recurse
		...
		# Create new folder shortcuts in the following directories: application program files. 
		...
		Foreach ($appShortcutDir in $appShortcutDirs) {
			Try {
				...
				New-Shortcut -Path "$appShortcutDir\$32bitAppInstallName\OpenGov Budgeting.lnk" -TargetPath https://controlpanel.opengov.com/ -IconLocation "$32bitAppInstallPath\Assets\Budgeting_icon.ico" -WindowStyle "Maximized"
				New-Shortcut -Path "$appShortcutDir\$32bitAppInstallName\OpenGov CIT.lnk" -TargetPath https://tuolumnecountyca.viewpointcloud.io -IconLocation "$32bitAppInstallPath\Assets\OpenGovCIT.ico" -WindowStyle "Maximized"
				New-Shortcut -Path "$appShortcutDir\$32bitAppInstallName\OpenGov Procurement.lnk" -TargetPath https://procurement.opengov.com -IconLocation "$32bitAppInstallPath\Assets\OpenGov_icon.ico" -WindowStyle "Maximized"
		...
		# Create new folder shortcuts in the following directories: Start Menu, & Desktop. 
		# If shortcut creation failed, log results. Exit script. 
		# Else, log results from check.
		Foreach ($folderShortcutDir in $folderShortcutDirs) {
			Try {
				Write-Log -Message "Creating application shortcut folder in $folderShortcutDirs."
				New-Shortcut -Path "$folderShortcutDir\$32bitAppInstallName.lnk" -TargetPath "${Env:ProgramFiles}\Tuolumne County\$32bitAppInstallName\$32bitAppInstallName" -IconLocation "$32bitAppInstallPath\Assets\Folder.ico"
				Write-Log -Message "Creating application shortcut folder in $folderShortcutDirs complete."
			}
			# If shortcut creation failed, log results. Exit script.
			Catch [System.Exception] {
				Write-Log -Message "Creating Start Menu & Desktop folder shortcuts failed with error: $_."
				Write-Log -Message "Exiting script with error."
				Exit-Script -ExitCode 1627
			}
		}
		...
		Foreach ($removeShortcutDir in $removeShortcutDirs) {
			...
			If (Test-Path -Path "$removeShortcutDir\$32bitAppUninstallName") {
				...
				Try {
					...
					Remove-Item -Path "${Env:ProgramData}\Microsoft\Windows\Start Menu\Programs\$32bitAppUninstallName*" -Force
					Write-Log -Message "Removing previous versions of $32bitAppUninstallName in $removeShortcutDir complete."
				}
				# If removal failed, log results. Exit script.
				Catch [System.Exception] {
